...
함수 합성 관점에서의 promise
란
promise는 비동기 상황에서 함수 합성을 안전하게 하기위해 사용되는 도구
연속적인 함수를 합성

비동기 값을 가지고 연속적인 함수 실행을 안전하게 하는 모나드

모나드를 설명하기 위해선 함수합성을 먼저 설명이 필요

상황에따라 안전하게 함수를 합성하기위해 모나드란 개념이 있고, 그 구현체중에 비동기 상황을 안전하게 합성하기 위한 도구가 promise다

자바스크립트는 동적 스크립트언어이며 타입을 사고하며 프로그래밍하는 언어가 아니기 때문에
대수 구조, 모나드의 개념이 잘 묻어나지 않음

모나드는 결국 함수 합성을 안전하게 하기 위한 도구 및 개념
...
만약에 String, List<String> 을 input으로 넣어도 잘 작동해야 한면?
```js
